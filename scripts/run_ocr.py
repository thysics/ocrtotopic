...
def process_single_file(args: Tuple[Path, Path, PDFTextExtractor]) -> bool:
    ...
    try:
        ...
        if output_file.exists():
            logging.info(f"{str(output_file)} already exists. Skip processing.")
            return True
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Extract text from PDFs using multiple threads"
    )
    parser.add_argument(
        "--input_directory", required=True, help="Directory containing PDF files"
    )
    parser.add_argument(
        "--output_directory", required=True, help="Directory to save text files"
    )
    ...
    extractor = PDFTextExtractor(
        device="cpu"
    )  # Use Metal Performance Shaders for M1 Mac
